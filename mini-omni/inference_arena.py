...
def load_model(ckpt_dir, device):
    ...
    # snacmodel = SNAC.from_pretrained("/mnt/user/bufan/model_cache/snac_24khz").eval().to(device)
...
def sort_key(filename):
    ...
    # 返回第一个匹配的数字，并转换为整数作为排序依据
    return int(numbers[0]) if numbers else 0


def test_infer(input_dir, out_dir):
    device = "cuda:0"
    ...
    test_audio_list = sorted(os.listdir(input_dir), key=sort_key)
    test_audio_list = [os.path.join(input_dir, path) for path in test_audio_list]
...
if __name__ == "__main__":
    ...
    for input_dir in list_subdirectories(base_path):
        output_dir = input_dir.replace(base_path, output_path)
        os.makedirs(output_dir, exist_ok=True)
        test_infer(input_dir, output_dir)
